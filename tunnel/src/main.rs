...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() < 4 {
        eprintln!(
            "Usage: {} [bind_to] [upstream] [hostname] [ca_file]",
            &args[0]
        );
    ...
    let (num_certificates, _) = load_ca(&mut client_config, Path::new(&args[3]))?;
    ...
    loop {
        let session = client.wait_for_session().await;
        if session.is_err() {
            continue;
        }

        let (meta, pipes) = session.unwrap();
